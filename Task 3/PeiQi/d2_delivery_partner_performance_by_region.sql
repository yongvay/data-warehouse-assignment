...
--   WHERE : Which destination regions have the strongest / weakest fulfilment?
--   WHO   : Which delivery companies perform best within each region?
--   WHAT  : What delivery outcomes and volume shares are being observed?
--   WHY   : Is lower delivery cost associated with weaker service performance?
--   WHEN  : How does on-time performance change month by month?
--
-- Facts / Dimensions:
--   DELIVERY_FACT
...
--
-- Cohort Rule:
--   The report is filtered by DELIVERY_FACT.ORDER_DATE_KEY so that all
...
--
--   The second measure intentionally keeps charges from all delivery records
--   in the numerator so cancellation / unsuccessful-delivery burden is visible.
-- ============================================================================
...
-- ============================================================================
-- EXHIBIT D2.1
-- WHERE: WHICH DESTINATION REGIONS CARRY THE GREATEST DELIVERY RISK?
-- ============================================================================

PROMPT ================================================================================================
PROMPT EXHIBIT D2.1 - WHERE: DELIVERY PERFORMANCE BY DESTINATION REGION
PROMPT Chart type : Supporting Table (No Separate Chart)
PROMPT Purpose    : Compares delivery volume, speed, cancellations and cost across regions.
PROMPT ================================================================================================
PROMPT
...
regional AS (
    SELECT
        ...
        100 * SUM(CASE WHEN delivery_status = 'Cancelled'
                       THEN 1 ELSE 0 END)
...
-- ============================================================================
-- EXHIBIT D2.2
-- WHO: WHICH DELIVERY PARTNER PERFORMS BEST WITHIN EACH REGION?
-- REQUIRED D2 SCORECARD
-- CHART: GROUPED BAR - AVG LEAD DAYS BY COURIER x REGION
-- ============================================================================
...
PROMPT Purpose    : Ranks delivery companies within each destination region.
PROMPT ================================================================================================
PROMPT
...
-- ============================================================================
-- EXHIBIT D2.3
-- WHAT: WHAT DELIVERY OUTCOMES OCCUR IN EACH REGION?
-- ============================================================================

PROMPT
PROMPT ================================================================================================
PROMPT EXHIBIT D2.3 - WHAT: DELIVERY STATUS MIX BY REGION
PROMPT Chart type : Supporting Table (No Separate Chart)
PROMPT Purpose    : Shows Delivered, In Transit, Pending and Cancelled outcomes by region.
PROMPT ================================================================================================
PROMPT

COLUMN address_region   HEADING 'Region'      FORMAT A20
COLUMN deliveries       HEADING 'Total'       FORMAT 999,990
COLUMN delivered        HEADING 'Delivered'   FORMAT 999,990
COLUMN in_transit       HEADING 'In Transit'  FORMAT 999,990
COLUMN pending          HEADING 'Pending'     FORMAT 999,990
COLUMN cancelled        HEADING 'Cancelled'   FORMAT 999,990
...
WITH status_summary AS (
    SELECT
        a.address_region,
        COUNT(*) AS deliveries,
        SUM(CASE WHEN df.delivery_status = 'Delivered'
                 THEN 1 ELSE 0 END) AS delivered,
        SUM(CASE WHEN df.delivery_status = 'In Transit'
                 THEN 1 ELSE 0 END) AS in_transit,
        SUM(CASE WHEN df.delivery_status = 'Pending'
                 THEN 1 ELSE 0 END) AS pending,
        SUM(CASE WHEN df.delivery_status = 'Cancelled'
...
SELECT
    ...
    pending,
    cancelled,
    ROUND(100 * delivered / NULLIF(deliveries, 0), 2)
        AS delivered_pct,
    ROUND(100 * cancelled / NULLIF(deliveries, 0), 2)
        AS cancelled_pct
FROM status_summary
ORDER BY
    cancelled_pct DESC,
...
-- ============================================================================
-- EXHIBIT D2.4
-- WHY: IS LOW COST ACTUALLY PRODUCING GOOD SERVICE?
-- ============================================================================

PROMPT
PROMPT ================================================================================================
PROMPT EXHIBIT D2.4 - WHY: COST VS SERVICE TRADE-OFF
PROMPT Chart type : Supporting Table (No Separate Chart)
PROMPT Purpose    : Compares each courier with its regional cost and on-time averages.
PROMPT ================================================================================================
PROMPT
...
COLUMN deliveries                  HEADING 'Deliveries'               FORMAT 999,990
COLUMN avg_delivery_charge         HEADING 'Avg Charge|(RM)'          FORMAT 999,990.00
COLUMN charge_gap_vs_region        HEADING 'Charge Gap|vs Region'     FORMAT 999,990.00
COLUMN on_time_pct                 HEADING 'On-Time|%'                FORMAT 990.99
COLUMN on_time_gap_vs_region       HEADING 'On-Time Gap|vs Region'    FORMAT 990.99
COLUMN cancelled_pct               HEADING 'Cancelled|%'              FORMAT 990.99
COLUMN cost_per_delivered_order    HEADING 'Cost / Delivered|(RM)'    FORMAT 999,990.00
...
metrics AS (
    ...
    FROM base
    GROUP BY address_region, company_name
),
benchmarked AS (
    SELECT
        m.*,
...
SELECT
    ...
    deliveries,
    ROUND(avg_delivery_charge, 2) AS avg_delivery_charge,
    ROUND(avg_delivery_charge - region_avg_charge, 2)
        AS charge_gap_vs_region,
    ROUND(on_time_pct, 2) AS on_time_pct,
    ROUND(on_time_pct - region_avg_on_time, 2)
        AS on_time_gap_vs_region,
    ROUND(cancelled_pct, 2) AS cancelled_pct,
    ROUND(cost_per_delivered_order, 2)
        AS cost_per_delivered_order
FROM benchmarked
ORDER BY
    address_region,
    on_time_gap_vs_region ASC,
    cancelled_pct DESC,
    company_name;
...
PROMPT
PROMPT ================================================================================================
PROMPT EXHIBIT D2.5 - WHEN: MONTHLY ON-TIME PERFORMANCE TREND
PROMPT Chart type : Line Chart
PROMPT Chart      : Monthly On-Time % by Delivery Company
PROMPT Purpose    : Reveals whether courier performance is stable or deteriorates over time.
PROMPT ================================================================================================
PROMPT
